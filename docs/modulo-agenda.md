...
**Estado:** Fase 1 completa — lógica de dominio y tests funcionales implementados; interfaz Livewire/Filament pendiente  
...
**Constraint de unicidad:** El modelo valida en capa de aplicación (`booted()` → evento `saving`) que no exista otro perfil con `activo = true` para la misma combinación `(usuario_id, centro_id)`. Si la validación falla, lanza `LogicException`. Los perfiles inactivos (histórico) no están sujetos a esta restricción.
...
| `tipo_evento` | string | Valor de catálogo (`catalogos_sistema`, grupo `tipo_evento_agenda`) |
| `titulo` | string | |
| `descripcion` | text nullable | |
| `fecha` | date | |
| `hora_inicio` | time | |
...
| `agregarProfesionales(array $ids)` | `array<usuarioId, Collection<Cita>>` | Convoca profesionales: bloquea sus slots `disponible` en la franja del evento (`→ bloqueado_evento`) y devuelve mapa de citas confirmadas afectadas. Los slots `reservado` no se tocan. Usa `syncWithoutDetaching` (idempotente). |
| `detectarConflictoEspacio()` | `bool` | Devuelve `true` si el `espacio_id` del evento solapa con otro evento en la misma fecha. No bloquea la creación; sirve para generar el aviso al supervisor. |

**Pivot `evento_usuario`:** `evento_agenda_id`, `usuario_id`, `confirmado` (boolean), `notas`

**Nota sobre conflictos de espacio:** Si el espacio ya está reservado por otro evento simultáneo, el sistema genera un aviso (`detectarConflictoEspacio()`) pero no bloquea la creación. No hay locking automático de espacios.
...
Calcula los slots disponibles para un profesional, un tipo de slot y un rango de fechas. No genera slots: consulta los ya materializados filtrando por estado. Implementado en `Modules/Agenda/app/Services/DisponibilidadService.php`.
...
- `incluirUrgencias = false` (defecto): solo estados `disponible`. Adecuado para canal externo y consultas ciudadanas.
- `incluirUrgencias = true`: incluye también `bloqueado_urgencia`. Para canal interno, supervisores y reasignaciones asistidas.
- Filtra siempre por `centro_id`, garantizando que los profesionales itinerantes solo tienen disponibilidad en el centro correcto.

### 3.2 CuadranteGeneratorService ✅

Genera el borrador de `LineaCuadrante` para un mes a partir del `HorarioCentro` vigente y los `PerfilHorarioProfesional` activos. Aplica las `ExcepcionProfesional` ya conocidas para el período. Implementado en `Modules/Agenda/app/Services/CuadranteGeneratorService.php`.

- `generarBorrador(CuadranteMes)`: crea `LineaCuadrante` por profesional por día laborable. Usa bulk `insert()` para eficiencia. Las excepciones preexistentes generan líneas con `anulada = true` (no slots).
- `generarYPublicarAutomaticamente(Centro, int $anyo, int $mes)`: crea el `CuadranteMes`, genera borrador, publica y materializa slots. Solo en modo `basico` (`generado_automaticamente = true`).

En modos `estandar` y `avanzado`, la propuesta es siempre un borrador que el supervisor debe revisar y publicar. La integración con IA (modo `avanzado`) está diferida.

### 3.3 SlotMaterializadorService ✅

Al publicar un `CuadranteMes`, genera los registros `Slot` a partir de las `LineaCuadrante`. Implementado en `Modules/Agenda/app/Services/SlotMaterializadorService.php`.

- Aplica los buffers del `HorarioCentro` (inicio y fin de jornada).
- Aplica la duración y el `porcentaje_urgencias` de cada `TipoSlot`. Los slots de urgencia (`floor` del porcentaje) se crean como `bloqueado_urgencia`; el resto como `disponible`.
- Excluye líneas con `anulada = true`.

### 3.4 GestionAusenciaService ✅

Servicio operativo que gestiona el flujo cuando un profesional no se presenta. Implementado en `Modules/Agenda/app/Services/GestionAusenciaService.php`.

- `procesarAusencia(int $usuarioId, int $centroId, Carbon $fecha)`: cancela las citas `confirmada` del profesional con `motivo_cancelacion = 'Ausencia del profesional'` y devuelve candidatos de reasignación. En modos `estandar`/`avanzado`: candidatos = slots `bloqueado_urgencia` de otros profesionales. En modo `basico`: candidatos = slots `disponible` de otros profesionales.
- `reasignar(Cita, Slot $destino, int $supervisorId, string $motivo)`: crea `ReasignacionCita`, actualiza la cita con el nuevo profesional/slot y marca el slot destino como `reservado`. El slot original permanece en su estado.

**Nota sobre el estado de las citas canceladas:** las ausencias sobrevenidas generan citas en estado `cancelada` (no `no_show_profesional`). El estado `no_show_profesional` queda reservado para incumplimientos injustificados; la distinción es relevante para estadísticas de gestión.

### 3.5 SlotExpirationJob ✅

Job programado que se ejecuta al final de cada día laboral. Implementado en `Modules/Agenda/app/Jobs/SlotExpirationJob.php`.

- `bloqueado_urgencia` + fecha pasada → `expirado`.
- `disponible` + fecha pasada → `no_ocupado`.
- `reservado` + fecha pasada + sin cita en estado `confirmada` o `completada` → `no_ocupado` (no-shows de ciudadano cuyo slot no fue liberado manualmente).

No realiza ninguna acción sobre profesionales ni sobre citas.
...
**Última ejecución:** 2026-05-20 — `php vendor/bin/phpunit --filter="Modules\\\\Agenda"`
**Resultado:** 45 tests — 45 pasan ✅ — 0 pendientes — 0 fallos ❌

| Área | Tests | Estado | Implementado con |
|---|---|---|---|
| PF-01 Configuración horario | 4/4 | ✅ | `HorarioCentro`, `SlotMaterializadorService` |
| PF-02 Perfil horario | 4/4 | ✅ | `PerfilHorarioProfesional::booted()`, `SlotMaterializadorService` |
| PF-03 Cuadrante mensual | 5/5 | ✅ | `CuadranteGeneratorService`, `SlotMaterializadorService` |
| PF-04 Slots y disponibilidad | 5/5 | ✅ | `DisponibilidadService`, `SlotExpirationJob`, `EventoAgenda::agregarProfesionales()` |
| PF-05 Ciclo de vida cita | 8/8 | ✅ | `CitaObserver`, `Cita::completar/cancelar/noShowCiudadano()` |
| PF-06 No-show ciudadano | 3/3 | ✅ | `Cita::noShowCiudadano()` |
| PF-07 No-show profesional | 5/5 | ✅ | `GestionAusenciaService`, `ExcepcionProfesionalObserver` |
| PF-08 Eventos de agenda | 4/4 | ✅ | `EventoAgenda::agregarProfesionales()`, `detectarConflictoEspacio()` |
| PF-09 Profesionales itinerantes | 2/2 | ✅ | `DisponibilidadService`, `ExcepcionProfesionalObserver` |
| PF-10 Integridad y casos límite | 5/5 | ✅ | `CuadranteGeneratorService`, `SlotMaterializadorService` |
...
**PF-02.3 ✅ — Solapamiento de perfiles en el mismo centro**
Contexto: se intenta crear un segundo perfil activo para el mismo profesional y centro.
Pasos: intentar guardar el segundo perfil con `activo = true`.
Esperado: el sistema lanza `LogicException` (validación en `PerfilHorarioProfesional::booted()` → evento `saving`). Los perfiles con `activo = false` para el mismo combo no están sujetos a esta restricción.

**PF-02.3b ✅ — Perfil inactivo no bloquea la creación de un activo**
Contexto: existe un perfil histórico con `activo = false` para el mismo profesional y centro.
Pasos: crear un nuevo perfil con `activo = true`.
Esperado: el sistema lo acepta sin error. El perfil inactivo no impide la creación del activo.
...
Contexto: profesional con citas confirmadas para hoy.
Pasos: el supervisor llama a `GestionAusenciaService::procesarAusencia()` para el profesional y fecha.
Esperado: las citas confirmadas pasan a estado `cancelada` con `motivo_cancelacion = 'Ausencia del profesional'`. El servicio devuelve las citas canceladas y los candidatos de reasignación (slots `bloqueado_urgencia` de otros profesionales en modo estándar/avanzado).
